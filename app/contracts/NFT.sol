...
contract NFT is ERC721URIStorage {
    ...
    function createSVGNFT(string memory _d_params) public returns (uint256) {
        tokenCounter = tokenCounter + 1;
        // TODO decode params to modify ayekilua
        string memory _svg = generateAyekiluaSVG(tokenCounter, _d_params);
    ...
    function generateAyekiluaSVG(uint256 tokenId, string memory _d_params) public pure returns (string memory _generatedSVG ) {
        return string(
            abi.encodePacked(
                ...
                '</style>',
                '</defs>',
                '<title id="ayekiluaTitle">Ayekilua ', uint2str(tokenId),_d_params,'</title>',
